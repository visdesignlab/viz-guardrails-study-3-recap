...
export function StripPlot({
    ...
    const nicedYScale = useMemo(() => {
        return d3.scaleLinear([margin.top, height - margin.bottom]).domain([selectedDataRange[1], selectedDataRange[0]]).nice();
    }, [height, selectedDataRange]);

    // const colorScale = useMemo(() => {
    //     return d3.scaleDiverging([+d3.min(allData.array('value')), 0, +d3.max(allData.array('value'))], ['#B22222',
    //         '#e5e5e5',
    //         '#4682b4']);
    // }, [allData]);
    ...
    return selection?.length===0 ? null : (
        <svg ref={ref} style={{height: '400px', width: '60px', overflow: 'visible'}}> 
            ...
            <rect y={yScale(selectedDataRange[1])} height={yScale(selectedDataRange[0]) - yScale(selectedDataRange[1])} x={margin.left - 10} width={10} opacity={0.1} fill="black"></rect>
            ...
            <text style={{fontSize: 10, dominantBaseline: 'middle', fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif'}} x={margin.left - 5} y={yScale(selectedDataRange[0])}>{textFormat(nicedYScale.domain()[1])}</text>
            <text style={{fontSize: 10, dominantBaseline: 'middle', fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif'}} x={margin.left - 5} y={yScale(selectedDataRange[1])}>{textFormat(nicedYScale.domain()[0])}</text>

            {data.map((d: any) => <rect opacity={.2} fill={+d[parameters.y_var] > 0 ? 'steelblue' : 'firebrick'} x={0} width={50} y={yScale(+d[parameters.y_var])} height={1}></rect>)}
            <line strokeWidth={2} stroke="black" x1={0} x2={50} y1={yScale(selectedDataRange[0])} y2={yScale(selectedDataRange[0])}></line>
            ...
            <path fill={'black'} opacity="0.1" d={`M${margin.left}, ${yScale(selectedDataRange[1])} L ${width + 20 + 33}, ${margin.top} L ${width + 20 + 33}, ${height - margin.bottom} L ${margin.left}, ${yScale(selectedDataRange[0])}`}></path>
        </svg>
    );
