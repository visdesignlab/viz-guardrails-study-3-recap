...
export function SingleTaskTimeline({participantData, width, height, selectedTask, currentNode, setCurrentNode, playTime, setPlayTime, isPlaying, setIsPlaying, setSelectedTask} : {participantData: ParticipantData, width: number, height: number, selectedTask?: string | null, currentNode: string | null, setCurrentNode: (node: string) => void, isPlaying: boolean, setIsPlaying: (b: boolean) => void, playTime: number, setPlayTime: (n: number, p: number) => void, setSelectedTask: (s: string) => void}) {
    ...
    const totalLength = useMemo(() => {
        return wholeXScale.domain()[1] - wholeXScale.domain()[0];
    }, [wholeXScale]);
